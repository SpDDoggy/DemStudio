...
- 第一阶段不把没有性能证据的代码提前迁入 Rust。

用户随后明确决定 Rust 化核心并重构 Fluent 无边框 UI，因此上述第一项和最后一项不再约束当前阶段。算法结果兼容、无云依赖和平台证据边界继续有效。

## 当前核心边界

- Rust Core：GeoTIFF/HGT/ASC 解码、NoData、统计、地理元数据、抽样、平滑和 GeoTIFF 编码。
- Tauri Host：窗口、原生文件路径、状态存储、IPC 与 Core 数据集生命周期。
- Web/Three.js：界面、GPU 渲染、相机、材质、实时预览与图像高度图。
- 非 Tauri 浏览器路径只作为兼容回退，不是正式桌面的主执行链。
...
- Tauri 宿主兼容层加载；
- Three.js Canvas 创建；
- ASC 文件输入事件与 Rust Core 调用；
- 4 × 4 栅格的 Rust 解析、抽样和地形重建；
- Rust GeoTIFF 编码输出 TIFF 魔数；
- Fluent 无边框标题栏与窗口 API；
- 页面无启动错误。
